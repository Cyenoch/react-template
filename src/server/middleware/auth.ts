...
export const authMiddleware = createMiddleware().server(async ({ next }) => {
  ...
  async function retireSession() {
    ...
    if (!session) {
      throw new Response('Session expired, please sign in again', {
        status: 401,
      });
    }
...
export const requireAuthMiddleware = createMiddleware()
  ...
  .server(async ({ next, context: { session, user, userId, sessionId } }) => {
...
export const getSessionFromContext = serverOnly(async () => {
  ...
  if (!session)
    throw new Error(
      'Session not initialized. (Please use this function within the request context)',
    );
...
export const getUserFromContext = serverOnly(async () => {
  ...
  if (!user)
    throw new Error(
      'User not initialized. (Please use this function within the request context)',
    );
  return await user();
});
